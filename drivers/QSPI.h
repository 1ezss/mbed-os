...
public:
     ...
     *
     *  @param hz SCLK frequency in hz (default = 4MHz)
     *  @returns
    ...
    /** Write a command to the QSPI peripheral to a specific address
     *
     *  @param instruction Instruction value to be used in instruction phase
     *  @param address Address to be accessed in QSPI peripheral
     *  @param alt Alt value to be used in instruction phase
     ...
     */
    int write_command(unsigned int instruction, unsigned int address, unsigned int alt);
     ...
     *    Returns 1 on successful command transaction and 0 if operation failed.
     */
    int perform_command_transaction(unsigned int instruction, const char *tx_buffer, size_t tx_length, const char *rx_buffer, size_t rx_length);
